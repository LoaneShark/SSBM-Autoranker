...
parser.add('-dn','--display_size',help='lowest placing shown on pretty printer output, or -1 to show all entrants (default 64)',default=64)
...
parser.add('-rw','--srank_running_avg_sigma', default = 0.1, help= 'Gaussian window sigma for running average weighting (default 0.1)')
parser.add('-rl','--srank_running_avg_step', default = 0.05, help= 'Step size for gaussian windows in running average calculation (default 0.05)')
...
def generate_db_str():
	db_list = [args.game,args.year,args.year_count]
	if args.current_db:
		if args.season_db:
			db_list.append('s')
		else:
